...
#if defined(NDEBUG)
#error "WBTC cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uWBTC) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 2500000 * COIN; //for swap, dev, first masternode, bounty and airdrop (only 100 users for one day)
    }
	else if (nHeight >= 201  && nHeight <= 1000){

        nSubsidy = 20 * COIN;
}

	else if (nHeight >= 1001  && nHeight <= 2000){

        nSubsidy = 10 * COIN;
}

	else if (nHeight >= 2001  && nHeight <= 5000){
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK() || blockValue == 0) // MN start after block 100
        return 0;
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // We use 7 blocks rather WBTCtrarily as it represents just under 10% of sustained network
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // WBTC: We use certain sporks during IBD, so check to see if they are
